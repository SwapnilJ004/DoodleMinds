...
export default function ScribbleGame() {
  ...
  const [chatMessages, setChatMessages] = useState<
    Array<{ player: string; message: string; isCorrect?: boolean }>
  >([]);
  ...
  const handleGuess = () => {
    if (!guessInput.trim() || !roomId || players[currentPlayerId]?.hasGuessed)
      return;
    const isCorrect =
      guessInput.toLowerCase().trim() ===
      currentWord.split(" ")[1]?.toLowerCase();

    push(ref(database, `rooms/${roomId}/chatMessages`), {
    ...
    if (isCorrect) {
      ...
      update(playerRef, {
        score: (players[currentPlayerId].score || 0) + pointsEarned,
        hasGuessed: true,
      });
    }
    setGuessInput("");
  ...
  const handleDrawingEnd = () => {
    if (currentDrawer !== currentPlayerId || !roomId || !currentPath.current)
      return;
    const newPath = {
    ...
    <View style={styles.lobbyContainer}>
      ...
      <View style={styles.joinSection}>
        <TextInput
          ...
          maxLength={15}
        />
        <TouchableOpacity style={styles.joinButton} onPress={handleCreateGame}>
        ...
        />
        <TouchableOpacity
          style={[styles.joinButton, styles.joinExistingButton]}
      ...
      <View style={styles.gameContainer}>
        <View style={styles.gameHeader}>
          ...
          <View style={styles.roundContainer}>
            <Text style={styles.roundText}>
              Round {round}/{Object.keys(players).length}
            </Text>
          </View>
        </View>
        <View style={styles.wordContainer}>
          {isDrawer ? (
            ...
            <Text style={styles.wordHint}>{drawerName} is drawing...</Text>
          )}
        </View>
        <View
          style={styles.canvas}
            ...
            <View>
              ...
              <View style={styles.guessContainer}>
                <TextInput
                  ...
                  editable={!players[currentPlayerId]?.hasGuessed}
                />
                <TouchableOpacity
                  style={styles.guessButton}
                  onPress={handleGuess}
                  disabled={players[currentPlayerId]?.hasGuessed}
                >
